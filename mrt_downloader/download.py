...
def index_files_for_rrcs(rrcs: Iterable[int], start_time: datetime.datetime, end_time: datetime.datetime) -> List[str]:
    ...
    for rrc in rrcs:
        now = start_time
        while True:
            index_url = BASE_URL_TEMPLATE.format(rrc=rrc, year=now.year, month=now.month)
...
async def process_rrc_index(session: aiohttp.ClientSession, entry: RrcIndexEntry) -> List[RisFileEntry]:
        ...
        if response.status != 200:
            ...
            for link in parser.links:
                ...
                if file_name.startswith('updates.') or file_name.startswith("bview."):
                    result.append(RisFileEntry(entry.rrc, file_name, urllib.parse.urljoin(entry.url, link)))
...
async def main_process(target_dir: Path, start_time: datetime.datetime, end_time: datetime.datetime, num_workers: int, bview_only: bool = False, update_only: bool = False, rrc: Optional[List[str]] = None):
    """Gather the list of update files per timestamp per rrc and download them."""
    matches = []
    if rrc is not None and len(rrc) > 0:
        click.echo(click.style(f"Downloading updates from _only_ RRCs {rrc}", fg='green'))
    ...
    async with aiohttp.ClientSession() as session:
        indexes = await asyncio.gather(*[process_rrc_index(session, index) for index in index_urls])
    ...
    for file in matches:
        ...
        await queue.put(Download(
            file.url,
            target_dir / f"rrc{file.rrc:02}-{file.file_name}"
        ))

    click.echo(click.style(f"Downloading {len(matches)} files on {num_workers} workers", fg='green'))

    async with aiohttp.ClientSession() as session:
        workers = [
            worker(session, queue) for i in range(num_workers)
        ]
