...
async def download_file(
    session: aiohttp.ClientSession, download: Download
) -> None:
    t0 = time.time()
    if download.target_file.is_file():
        ...
        async with session.head(download.url) as response:
            ...
            if content_length and last_modified:
                ...
                if stat.st_size == int(content_length) and stat.st_mtime == last_modified_date.timestamp():
                    LOG.debug(
...
async def worker(
    session: aiohttp.ClientSession, queue: asyncio.Queue[Download]
) -> int:
    processed = 0
